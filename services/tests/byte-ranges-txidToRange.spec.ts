...
describe('Orphan data in DB tests', ()=>{
	it('should throw an error when an ans104 parent is not found', async()=> {
		...
		try{
			 ...
			 * */
			expect(e.message).eq(`Parent ${badParent} not found using https://arweave.net/graphql or https://arweave-search.goldsky.com/graphql. id CagF6yAfaqIUYi0IHJ1FwmkdMqBxjVJBntyD1X1QF1U`)
		}
	}).timeout(0)
})