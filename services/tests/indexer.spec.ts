jest.mock('../src/common/utils/gql-height', ()=>({
	gqlHeight: jest.fn(),
}))
jest.mock('../src/indexer/indexer', ()=>{
	return {
		...jest.requireActual('../src/indexer/indexer'),
		sleep: jest.fn().mockResolvedValue(null),
		readPosition: jest.fn(),
	}
})
import { gqlHeight } from '../src/common/utils/gql-height'
import { topAvailableBlock, sleep, readPosition } from '../src/indexer/indexer'
...
describe('indexer tests', ()=>{
	...
	it('tests topAvailableBlock does not let pass2 get ahead of pass1', async()=>{
		...
		mockReadPosition.mockImplementation(async(indexName: string)=> indexName === 'indexer_pass1' ? posn1++ : posn2++)
		console.log({readPosition})
		console.log({sleep})

		// console.log('gqlHeight', gqlHeight)
		const mockGqlHeight = gqlHeight as jest.MockedFunction<typeof gqlHeight>
		mockGqlHeight.mockResolvedValue(weave)
		...
		const pass1 = await topAvailableBlock(posn1, 0, 'http://fake.url', 'indexer_pass1') //pass1 confirmations 0
		const pass2 = await topAvailableBlock(weave - 15, 15, 'http://fake.url', 'indexer_pass2')

		console.log({pass1, pass2})

		// expect(fakeGqlHeight.callCount).toBeGreaterThan(0)
		expect(pass1).toEqual(weave)
		expect(pass2).toEqual(pass1 - 15)
		expect(pass2).toBeLessThan(pass1)
